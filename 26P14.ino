...
#define _DIST_MIN 90                       //[3164] 최소 측정 거리 10mm로 고정 
...
// Servo range
#define _DUTY_MIN 1580    //[3148]  서보의 가동 최소 각도(0)
#define _DUTY_NEU 1780      //[3150] servo neutral position (90 degree)
#define _DUTY_MAX 1980                // [3169] 서보의 최대 가동 각도(180º)
...
float dist_raw, dist_cali, dist_ema;    //[3160] 실제 거리측정값과 ema필터를 적용한 값을 저장할 변수
...
// move servo to neutral position
  myservo.writeMicroseconds(1775); // [3147] 서보를 중간으로 이동
...
////////////////////
  ...
  // get a distance reading from the distance sensor
    float dist_filtered = ir_distance_filtered();  // [3157] 적외선 선세로 측정한 값에 필터를 적용한 값
  ...
  // keep duty_target value within the range of [_DUTY_MIN, _DUTY_MAX]
    ...
    if(error_prev - error_curr < 100 || error_prev - error_curr > -100){
...
float dist_calc(float dist_value){
  if(dist_raw<=255){
    ...
    return constrain(400 + 10.0 / (1600 - 1000) * (dist_raw - 838), 400, 410);
  }
}

float f(float control){
  return _DUTY_NEU + control;
}
